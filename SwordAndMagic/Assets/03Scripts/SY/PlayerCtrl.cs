...
public class PlayerCtrl : MonoBehaviour
{
    ...
    */
    ...
    void Awake()
    {
        ...
        EXP_Bar.maxValue = 100;
        EXP_Bar.value = playerExp;
        EXP_Text.text = EXP_Bar.value + " / " + EXP_Bar.maxValue;
    ...
    void GetInput()
    {
        ...
        {
            if(PCstatusUI.activeSelf == false && Time.timeScale == 1)
            {
                Time.timeScale = 0;
                PCstatusUI.SetActive(true);
            }
            else if(PCstatusUI.activeSelf == true && Time.timeScale == 0)
            {
                Time.timeScale = 1;
                PCstatusUI.SetActive(false);
            }
            EXP_Bar.value = playerExp;   //MaxHp는 따로 늘어날때 재보정하므로 xur/max는 안함
            EXP_Text.text = EXP_Bar.value + " / " + EXP_Bar.maxValue;           
        }
    }
    #region Movement
